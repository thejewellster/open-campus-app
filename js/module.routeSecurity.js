(function(angular) {
   ...
   RouteSecurityManager.prototype = {
      ...
      _authRequiredRedirect: function(route, path) {
         ...
         else if (route.passwordRequired && !this._authenticated){
            if (route.pathTo === undefined) {
               this._redirectTo = this._location.path();
            } else {
               this._redirectTo = route.pathTo === path ? "/" : route.pathTo;
         ...
         } else if (route.authRequired && this._rootScope.auth.user.provider == 'password') {
            this._redirect('/post');
         } else if (route.passwordRequired && this._rootScope.auth.user.provider == 'facebook') {
            this._redirect('/account');
         } else if( this._authenticated && (this._location.path() === this._loginPath || this._location.path() === '/password') ) {
            this._redirect('/');
         }
      }
   };
